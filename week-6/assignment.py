...
def signin():
    ...
    if signName=="" or signAccount=="" or signPassword=="":
        ...
        mycursor.execute("SELECT username FROM member WHERE username='"+signAccount+"'")
...
def login():
        ...
        mycursor.execute("SELECT name,username,password FROM member WHERE username='"+account+"' AND password='"+password+"'")
